/* Lunde V6 — customer password reset */
(function () {
  var L = window.lunde || {};
  ...
  if (token) {
    ...
    document.getElementById("resetIntro").textContent = "Choose a new password for your account.";
  }
  ...
  requestForm.addEventListener("submit", async function (e) {
    ...
    var r = null;
    try { r = await L.requestPasswordReset(email); } catch (err) { r = { ok: false, error: "Something went wrong. Try again." }; }
    if (r && r.ok) {
      requestStatus.innerHTML = "If an account exists, password reset instructions have been sent." + (r.devResetUrl ? '<br><a href="' + r.devResetUrl + '">Open local reset link</a>' : "");
  ...
  resetForm.addEventListener("submit", async function (e) {
    ...
    var r = null;
    try { r = await L.resetCustomerPassword(token, password); } catch (err) { r = { ok: false, error: "Something went wrong. Try again." }; }
    if (r && r.ok) { location.href = "/account/login?reset=1"; return; }
