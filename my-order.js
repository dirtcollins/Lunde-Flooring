/* Lunde V6 — order detail / tracking */
(function () {
  var L = window.lunde;
  var orderById = L.orderById, productById = L.productById, cartonsFor = L.cartonsFor, sqftPerCarton = L.sqftPerCarton,
  ...
  if (!order) {
    page.innerHTML = '<div class="v6-empty"><p class="eyebrow">Order</p><h1 class="display">Order not found.</h1><p>We couldn’t find that order. Check your account for your order history.</p><a class="btn lg" href="/account">My account</a></div>';
    return;
  }
  ...
  Object.keys(order.items).forEach(function (pid) {
    ...
    if (e.sqft > 0) {
      ...
      lines += '<div class="v6-line"><a class="v6-line-img" href="./product.html?slug=' + p.slug + '" style="background-image:url(\'' + window.lunde.thumb(p.mainImage) + '\')"></a>' +
        '<div class="v6-line-info"><strong>' + p.title + '</strong><span>' + cartons + ' cartons · ' + money(cartonPrice(p)) + '/carton · ' + e.sqft + ' sq. ft.</span></div>' +
  ...
  var stepper = cancelled ? '' : '<div class="mo-stepper">' + STATUSES.map(function (s, i) {
    return '<div class="mo-step' + (i <= activeIdx ? " done" : "") + '">' + STATUS_LABELS[s] + '</div>';
  }).join("") + '</div>';

  var nextStatus = STATUSES[activeIdx + 1];
  var banner = cancelled
    ? '<div class="mo-banner"><i style="background:#9a4a2e;animation:none"></i><div><strong>Cancelled</strong><span>' + NOTIFY.cancelled + '</span></div></div>'
  ...
  page.innerHTML =
    '<a class="mo-back" href="/account"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M19 12H5M11 6l-6 6 6 6"></path></svg>All orders</a>' +
    '<div class="mo-head" data-screen-label="Order ' + order.id + '"><div><p class="eyebrow">Order ' + order.id + '</p><h1 class="display">' + (cancelled ? "Order cancelled" : STATUS_LABELS[order.status]) + '.</h1><span class="acct-when">Placed ' + fmtDate(order.createdAt) + '</span></div>' +
      '<span class="status-badge" data-status="' + order.status + '"><i></i>' + STATUS_LABELS[order.status] + '</span></div>' +
    stepper +
    '<div class="mo-layout"><div>' +
      ...
      '<div class="v6-sum" style="position:static;margin-top:24px">' +
        ...
        '<div class="v6-sumtotal"><span>Total</span><b>' + money(t.total) + '</b></div>' +
        '<button class="btn lg" type="button" id="reorderBtn">Reorder these items</button>' +
      '</div>' +
    '</div><aside class="mo-info">' +
      '<div class="mo-info-card"><h4>Delivery</h4>' + deliveryCard + '</div>' +
      '<div class="mo-info-card"><h4>Contact</h4><p><b>' + esc(c.name || "Guest") + '</b>' + (c.company ? '<br>' + esc(c.company) : "") + (c.email ? '<br>' + esc(c.email) : "") + (c.phone ? '<br>' + esc(c.phone) : "") + (c.project ? '<br>Project: ' + esc(c.project) : "") + '</p></div>' +
      '<div class="mo-info-card"><h4>Updates</h4><div class="mo-timeline">' + timeline + '</div></div>' +
    '</aside></div>';

  document.getElementById("reorderBtn").addEventListener("click", function () {
    if (L.reorderToCart) { var n = L.reorderToCart(order.id); if (L.showToast) L.showToast(n ? "Items added to your cart" : "Those items are no longer available"); }
  });
})();
